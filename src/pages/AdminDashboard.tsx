...
import { processSellerPayout } from '../utils/paymentService';
...
export default function AdminDashboard() {
  ...
  const [pendingPayouts, setPendingPayouts] = useState<Order[]>([]);
  const [completedPayouts, setCompletedPayouts] = useState<Order[]>([]);
  ...
  const loadOrders = () => {
    ...
    setPendingPayouts(orders.filter((order: Order) => 
      order.status === 'paid' && !order.paymentTransferredToSeller
    ));
    setCompletedPayouts(orders.filter((order: Order) => 
  ...
  const handleApprove = (productId: string) => {
    ...
    const updatedProduct = { ...product, pending: false, featured: true } as Product;
    ...
    const updatedPending = pendingProducts.filter((p) => p.id !== productId);
  ...
  const handlePayout = async (order: Order) => {
    try {
      const success = await processSellerPayout(order);
      if (success) {
        // Update order status
        const orders = JSON.parse(localStorage.getItem('orders') || '[]');
        const updatedOrders = orders.map((o: Order) => 
          o.id === order.id 
            ? { ...o, paymentTransferredToSeller: true, sellerTransactionId: crypto.randomUUID() }
            : o
        );
        localStorage.setItem('orders', JSON.stringify(updatedOrders));
        loadOrders();
  ...
  return (
    <div className="space-y-8">
                ...
                <div key={order.id} className={`${theme === 'dark' ? 'bg-gray-700' : 'bg-gray-100'} p-6 rounded-lg`}>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="mt-2 p-2 bg-purple-500/10 rounded-lg">
                      ...
                      <p>Google Pay Number: {ADMIN_GPAY_NUMBER}</p>
                    </div>
                  </div>
                  <div className="mt-4 grid grid-cols-2 gap-4">
                    <button
      ...
      <section className={`${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} p-8 rounded-lg`}>
        ...
        <div className="space-y-6">
          <section>
            <h3 className="text-xl font-semibold mb-4">Pending Payouts</h3>
              ...
              <div key={order.id} className="bg-white p-4 rounded-lg shadow mb-4">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p>Seller Amount (90%): ₹{order.sellerAmount}</p>
                    <p>UPI Transaction ID: {order.upiTransactionId}</p>
                  </div>
                  <button
          ...
          <section>
            <h3 className="text-xl font-semibold mb-4">Completed Payouts</h3>
            {completedPayouts.map(order => (
              <div key={order.id} className="bg-gray-50 p-4 rounded-lg shadow mb-4">
                <p>Order ID: {order.id}</p>
                <p>Amount: ₹{order.totalAmount}</p>
                <p>Seller Amount Paid: ₹{order.sellerAmount}</p>
                <p>Seller Transaction ID: {order.sellerTransactionId}</p>
              </div>
            ))}
          </section>
        </div>
